...
namespace AviFile
{
	public class VideoStream : AviStream
	{
        ...
        //eu Isaque Neves alterei esta parte
		...
		private void CreateCompressedStream(){
            ...
            //A função avifil32.dll AVISaveOptions é que abre a janela opções de compressão e retorna um struct AVICOMPRESSOPTIONS

            /* MessageBox.Show(

                 " fccType: " + options.fccType.ToString()+
                 " fccHandler: " + options.fccHandler.ToString() +
                 " dwKeyFrameEvery: " + options.dwKeyFrameEvery.ToString() +
                 " dwQuality: " + options.dwQuality.ToString() +
                 " dwFlags: " + options.dwFlags.ToString() +
                 " dwBytesPerSecond: " + options.dwBytesPerSecond.ToString() +
                 " lpFormat: " + options.lpFormat.ToString() +
                 " cbFormat: " + options.cbFormat.ToString() +
                 " lpParms: " + options.lpParms.ToString() +
                 " cbParms: " + options.cbParms.ToString() +
                 " dwInterleaveEvery: " + options.dwInterleaveEvery.ToString() 
            ...
            //parametros para renderizar o AVI com x264vfw
            /*  fccType: 0
                fccHandler: 875967096
                dwKeyFrameEvery: 0
                dwQuality: 0
                dwFlags: 8
                dwBytesPerSecond: 0
            ...
            options.fccType = 0;
            options.fccHandler = 875967096;
            options.dwKeyFrameEvery = 0;
            options.dwQuality       = 0;  // 0 .. 10000
            options.dwFlags         = 8;  // AVICOMRPESSF_KEYFRAMES = 4
            options.dwBytesPerSecond= 0;
            options.lpFormat        = new IntPtr(0);
            options.cbFormat        = 0;
            options.lpParms         = new IntPtr(0);
            options.cbParms         = 9336;
